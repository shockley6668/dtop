...
namespace Cpu {
	...
	extern string current_governor;
...
namespace Bpu {
	...
	extern string platform_type; // "rdks600", "rdks100", "rdkx5", "rdkx3"
	extern int bpu_count;

	struct bpu_info {
		vector<string> names;
		vector<bool> temperature_only;
		vector<deque<long long>> usage;
		vector<deque<long long>> temp;
		deque<long long> aggregate_usage;
		deque<long long> aggregate_temp;
		long long temp_max = 0;
	...
	auto collect(const bool no_update=false) -> bpu_info&;

	//* Draw contents of BPU box using <bpu> as source
	string draw(const bpu_info& bpu, const bool force_redraw=false, const bool data_same=false);
