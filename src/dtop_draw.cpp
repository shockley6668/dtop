...
#include <dtop_draw.hpp>
#include <dtop_config.hpp>
#include <dtop_theme.hpp>
#include <dtop_shared.hpp>
#include <dtop_tools.hpp>
#include <dtop_input.hpp>
#include <dtop_menu.hpp>
...
namespace Cpu {
	...
	Draw::Meter bpu_meter;
	vector<Draw::Graph> bpu_core_graphs;
	vector<Draw::Graph> bpu_temp_graphs;
	Draw::Graph bpu_graph_upper;
	Draw::Graph bpu_graph_lower;
	...
	string draw(const cpu_info& cpu, const bool force_redraw, const bool data_same) {
		...
		if (redraw) {
			...
			//? CPU调频策略切换按钮
			// 首次初始化时读取当前调频策略
			if (current_governor.empty()) {
				try {
					if (std::filesystem::exists("/sys/devices/system/cpu/cpufreq/policy0/scaling_governor")) {
						std::ifstream current_file("/sys/devices/system/cpu/cpufreq/policy0/scaling_governor");
						getline(current_file, current_governor);
						if (not current_governor.empty() and current_governor.back() == '\n') {
							current_governor.pop_back();
						}
					}
					if (current_governor.empty()) {
			...
			int update_button_width = update.size() + 8; // "- XXXms +" 的宽度
			
			// 绘制CPU调频策略按钮
			string governor_button_text = "";
			int governor_button_width = 0;
			if (not current_governor.empty() and current_governor != "unknown") {
				string display_gov = current_governor;
				if (display_gov.size() > 12) display_gov = display_gov.substr(0, 12); 
				
				// 根据不同策略选择不同颜色
				string gov_color = "hi_fg"; 
				if (current_governor == "performance") {
					gov_color = "hi_fg"; // 高性能
				} else if (current_governor == "powersave") {
					gov_color = "proc_misc"; // 省电
				} else if (current_governor == "ondemand") {
					gov_color = "temp_mid"; // 按需
				} else if (current_governor == "conservative") {
					gov_color = "inactive_fg"; // 保守 
				} else if (current_governor == "schedutil") {
					gov_color = "title"; 
				} else if (current_governor == "userspace") {
					gov_color = "selected_fg"; 
				}
				
				governor_button_text = display_gov;
				governor_button_width = governor_button_text.size() + 2; 
				int gov_button_x = x + width - update_button_width - governor_button_width - 3; 
				
				out += Mv::to(button_y, gov_button_x) + title_left 
					+ Theme::c(gov_color) + Fx::b + governor_button_text + Fx::ub 
					+ title_right;
				
				Input::mouse_mappings["cpu_governor"] = {button_y, gov_button_x + 1, 1, governor_button_width - 2};
			}
			...
			//? CPU Graphs & meters 
			graph_upper = Draw::Graph{cpu_graph_width - 2, graph_up_height, "cpu", cpu.cpu_percent.at(graph_up_field), graph_symbol, false, true};
			cpu_meter = Draw::Meter{b_width - (show_temps ? 23 - (b_column_size <= 1 and b_columns == 1 ? 6 : 0) : 11), "cpu"};
			if (not single_graph)
				graph_lower = Draw::Graph{cpu_graph_width - 2, graph_low_height, "cpu", cpu.cpu_percent.at(graph_lo_field), graph_symbol, Config::getB("cpu_invert_lower"), true};
			...
			if (show_bpu) {
				...
				for (const auto& core_data : bpu.usage) {
			...
			if (mid_line) {
				out += Mv::to(y + graph_up_height + 1, x) + Fx::ub + Theme::c("cpu_box") + Symbols::div_left + Theme::c("div_line")
					+ Symbols::h_line * (width - 2) + Symbols::div_right
					+ Mv::to(y + graph_up_height + 1, x + (width / 2) - ((graph_up_field.size() + graph_lo_field.size()) / 2) - 4)
		...
		if (show_bpu and not bpu.usage.empty()) {
			const int bpu_graph_x = bpu_b_x + bpu_b_width + 1;
			out += Mv::to(y + 1, bpu_graph_x) + bpu_graph_upper(bpu.usage.at(0), (data_same or redraw));
			if (not single_graph and bpu.usage.size() > 1)
				out += Mv::to(y + graph_up_height + 1 + (mid_line ? 1 : 0), bpu_graph_x) + bpu_graph_lower(bpu.usage.at(1 % bpu.usage.size()), (data_same or redraw));
		}
		...
		if (Config::getB("show_cpu_freq") and not cpuHz.empty())
			out += Mv::to(b_y, b_x + b_width - 10)
				+ Fx::ub
				+ Theme::c("div_line")
				+ Symbols::h_line * (7 - cpuHz.size())
				+ Symbols::title_left
				+ Fx::b
				+ Theme::c("title")
				+ cpuHz
		...
		out += Mv::to(b_y + 1, b_x + 1)
			+ Theme::c("main_fg")
			+ Fx::b + "CPU "
			+ cpu_meter(cpu.cpu_percent.at("total").back())
			+ Theme::g("cpu").at(cpu.cpu_percent.at("total").back())
			+ rjust(to_string(cpu.cpu_percent.at("total").back()), 4)
			+ Theme::c("main_fg") + '%';
		...
		//? 核心文本和图形
		int cx = 0, cy = 1, cc = 0;
		int cpu_right_boundary = b_x + b_width - 1;
		int line_width_needed = (show_temps ? 12 : 6);

		for (const auto& n : iota(0, Shared::coreCount)) {
			if (b_x + cx + line_width_needed >= cpu_right_boundary) break;
			if (cy >= b_height - 2) break;

			out += Mv::to(b_y + cy + 1, b_x + cx + 1)
				+ Theme::c("main_fg")
				+ (Shared::coreCount < 100 ? Fx::b + 'C' + Fx::ub : "")
				+ ljust(to_string(n), 2);

			out += Theme::c("inactive_fg")
				+ graph_bg * 10 
				+ Mv::l(10) 
				+ Theme::g("cpu").at(cpu.core_percent.at(n).back())
				+ core_graphs.at(n)(cpu.core_percent.at(n), data_same or redraw); 

			out += ' '
				+ rjust(to_string(cpu.core_percent.at(n).back()), 3)
				+ Theme::c("main_fg") + '%';
		...
		if (show_bpu and not bpu.usage.empty()) {
			...
			if (bpu_b_x + 1 + bpu_header_width <= bpu_right_boundary) {
				out += Mv::to(bpu_b_y + 1, bpu_b_x + 1)
					+ Theme::c("main_fg")
					+ Fx::b + "BPU "
					+ bpu_meter(bpu_total_usage)
					+ Theme::g("temp").at(bpu_total_usage)
					+ rjust(to_string(bpu_total_usage), 4)
					+ Theme::c("main_fg") + '%';
			}
		...
		//? Block 3: BPU card drawing (if BPU is available)
...
namespace Draw {
	void calcSizes() {
		...
		const bool show_bpu = Bpu::has_bpu;
			...
			// Calculate card dimensions for 4-block layout (强制CPU双列显示)
			b_columns = 2; // 强制CPU仅双列显示，与绘制代码保持一致
			b_column_size = 1; // 强制启用宽图表模式
			
			// 为CPU和BPU卡片分配固定且安全的宽度 (针对双列优化)
			int max_card_width = show_bpu ? min(45, width / 3) : min(60, width / 2); // 为双列CPU增加更多宽度
			int min_graph_width = 10; // 减少图表区域占用，给双列CPU更多空间
			
			// 计算实际可用的卡片总宽度
			int total_available = width - (2 * min_graph_width) - 6; // 预留图表宽度和边距
			int total_card_width = show_bpu ? min(total_available, max_card_width * 2) : min(total_available, max_card_width);

			// 分配卡片宽度
			b_width = show_bpu ? total_card_width / 2 : total_card_width;
			bpu_b_width = show_bpu ? total_card_width / 2 : 0;
			b_height = min(height - 2, (int)ceil((double)Shared::coreCount / b_columns) + 4);
			bpu_b_height = b_height;

			// 计算图表宽度（剩余空间）
			int remaining_width = width - total_card_width - 6;
			cpu_graph_width = remaining_width / 2;
			bpu_graph_width = show_bpu ? remaining_width / 2 : 0;

			// Position calculation for 4-block layout: [CPU_Graph][CPU_Card][BPU_Card][BPU_Graph]
			int start_y = y + ceil((double)(height - 2) / 2) - ceil((double)b_height / 2) + 1;
			
			// CPU card position (block 2)
			b_x = x + cpu_graph_width + 3;
			b_y = start_y;
			
			// BPU card position (block 3) 
			bpu_b_x = show_bpu ? b_x + b_width + 1 : 0;
			bpu_b_y = start_y;
			...
			const string cpu_title = uresize((custom.empty() ? Cpu::cpuName : custom), b_width - 8);
