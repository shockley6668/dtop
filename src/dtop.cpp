...
void argumentParser(const int& argc, char **argv) {
	for(int i = 1; i < argc; i++) {
		...
		if (is_in(argument, "-h", "--help")) {
			cout 	<< "usage: dtop [-h] [-v] [-/+t] [--utf-foce] [--debug]\n\n"
					<< "optional arguments:\n"
			...
			cout << "dtop version: " << Global::Version << endl;
			exit(0);
		}
...
int main(int argc, char **argv) {
	...
	for (const auto& env : {"XDG_CONFIG_HOME", "HOME"}) {
		if (getenv(env) != NULL and access(getenv(env), W_OK) != -1) {
			Config::conf_dir = fs::path(getenv(env)) / (((string)env == "HOME") ? ".config/dtop" : "dtop");
			break;
		}
	}
	if (Config::conf_dir.empty()) {
		...
		if (std::error_code ec; not fs::is_directory(Config::conf_dir) and not fs::create_directories(Config::conf_dir, ec)) {
			cout 	<< "WARNING: Could not create or access dtop config directory. Logging and config saving disabled.\n"
					<< "Make sure $XDG_CONFIG_HOME or $HOME environment variables is correctly set to fix this." << endl;
		}
		else {
			Config::conf_file = Config::conf_dir / "dtop.conf";
			Logger::logfile = Config::conf_dir / "dtop.log";
	...
	//? Try to find global dtop theme path relative to binary path
	...
	if (std::error_code ec; not Global::self_path.empty()) {
		Theme::theme_dir = fs::canonical(Global::self_path / "../share/dtop/themes", ec);
	...
	if (Theme::theme_dir.empty()) {
		for (auto theme_path : {"/usr/local/share/dtop/themes", "/usr/share/dtop/themes"}) {
